...
_main() {
	...
	# skip setup if they aren't running mysqld-debug or want an option that stops mysqld
	if ([ "$1" = 'mysqld-debug' ] || ([ "$1" = "gdbserver" ] && [ "$3" = 'mysqld-debug' ]) ) && ! _mysql_want_help "$@"; then
		mysql_note "Entrypoint script for MySQL Server ${MYSQL_VERSION} started."
		mysql_check_config "mysqld-debug"
		# Load various environment variables
		docker_setup_env "mysqld-debug"
		docker_create_db_directories
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			docker_init_database_dir "mysqld-debug"

			mysql_note "Starting temporary server"
			docker_temp_server_start "mysqld-debug"
			mysql_note "Temporary server started."
